            ...
            for x in fp:
                ...
                if var == "NAME":
                    ...
                    if evjs is not None and evjs.is_defined:
                        # Increase the range for analog inputs for better precision
                        ...
                        for i,k in enumerate(keys):
                            ...
                            if evkb is not None and evkb.is_defined and devkb.has(evkb):
                                ...
                                if evjs.type == libevdev.EV_ABS:
                                    # Handle analog inputs with full range
                                    if var in ["ABS_Z", "ABS_RZ"]:  # Triggers
                                        mapping[evkb, 0] = ai.minimum
                                        mapping[evkb, 1] = ai.maximum
                                    else:  # Sticks
                                        if i >= len(keys) // 2:
                                            mapping[evkb, 1] = ai.maximum
                                        else:
                                            mapping[evkb, 1] = ai.minimum
